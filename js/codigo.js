...
function datos(nombre, apellido) {
    let datosPersonales;
    datosPersonales = "Muchas Gracias " + nombre + " " + apellido + " por comprar en nuestro negocio";
    return datosPersonales;
}
window.onload = function () {
    alert("Su compra ha sido un extio :)");
    alert("Acontinuacion le pediremos sus datos personales");
    let nom = prompt("Escriba su nombre:");
    let apelli = prompt("Escriba su apellido");
    let edad = prompt("Digite su edad");


    document.getElementById("mensaje3").innerHTML = `<h4>${datos(nom, apelli, edad)}</h4>`
}